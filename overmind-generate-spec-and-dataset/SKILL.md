...
- **Use** after `/overmind-register-agent` when `eval_spec.json` / `dataset.json` / `policies.md` are missing, out of sync with the entrypoint, or need repair before optimization.
- **Do not use** as a substitute for registration (no callable in `.overmind/agents.toml`) or for the optimize loop itself.
- **Do not use** `/overmind-optimise-agent` to “fix” broken registration or spec artifacts unless `overmind optimize-step` already ran and reported a concrete issue — prefer this skill or register first.

### Example (short)

User: *“Generate eval spec + dataset for `hotel-agent`.”* You resolve the registered entrypoint, lock I/O keys from AST (with low-confidence fallbacks where needed), write preview `eval_spec` / policy, get save approval, generate the dataset to **`_preview_dataset.json` only**, ask replace/append/backup if `dataset.json` exists, smoke-check, then point to `/overmind-optimise-agent`.
...
- **Entrypoint scope (default)**: Treat the registered Overmind entrypoint as a **fixed harness** — keep it out of `optimizable_paths`; place it in `exclude_paths` or `fixed_elements` in new eval specs. **Legacy specs:** if an existing `eval_spec.json` already lists the entrypoint as optimizable, warn the user that this is **not** the recommended default, proceed only if Overmind accepts the config, and recommend `/overmind-generate-spec-and-dataset` repair after optimization to move optimizable behavior into non-entrypoint modules.
...
- [ ] Step 8: Generate dataset to **`_preview_dataset.json` only**; enforce schema agreement (never write `dataset.json` until Step 9)
...
If returns are built indirectly (`model_dump()`, serializers, dataclass `asdict()`, variables, helpers, or branches you cannot resolve statically), **do not invent keys**: mark the output schema **low-confidence**, inspect call sites / return helpers / tests, and ask the user or widen the AST pass before locking `output_fields`.
...
**Sibling packages (batched):** Present the sibling list in one table and ask **once** (single `AskQuestion` or one chat response) for each row: *Optimizable* / *Context only* / *Exclude*. Never silently exclude a sibling package.
...
Also ask **once** (batch into a single question when possible):
...
### Step 8 — Generate the dataset (preview path only)
...
**Synthetic model (no silent vendor default):** Resolve `MODEL` for `generate_diverse_synthetic_data` in this order:

1. `SYNTHETIC_DATAGEN_MODEL` in the process environment or `.overmind/.env` — value must be a **non-empty** LiteLLM model id after trim and must pass the same placeholder rules as other skills (`<set-me>`, `changeme`, etc. are invalid).
2. If unset or placeholder, **stop** dataset generation and obtain a model id from the user (`AskQuestion` or chat), then export it for the runner process or append `SYNTHETIC_DATAGEN_MODEL=<id>` to `.overmind/.env` without clobbering unrelated keys.
3. **Never** substitute `openai/gpt-4o` or any other default model when the user has not explicitly provided a model string for this run.
...
print(f"Saved {len(cases)} cases to {preview}; dropped {len(dropped)}")
```
...
- Confirmation that the entrypoint file is **out of** `optimizable_paths` in the saved spec (or that the user accepted a legacy exception and was warned).
...
- **Entrypoint appears in optimizable scope**: In **repair mode**, remove it and place it in excluded or fixed scope; warn that optimization should target native agent modules instead. If the user refuses because an old spec requires it, document the risk and align with `/overmind-optimise-agent` legacy-compat rules.
...
- **`SYNTHETIC_DATAGEN_MODEL` unset or placeholder**: The datagen runner exits without a default vendor model. Set `SYNTHETIC_DATAGEN_MODEL` in `.overmind/.env` (or export for the process) or obtain a LiteLLM id from the user before running `_datagen_runner.py`.
...
- Never put the registered entrypoint in `optimizable_paths` in **new** specs; repair existing specs that violate this when the user agrees.
