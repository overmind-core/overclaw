...
| Key                      | Type   | When present                                              | Meaning                                                                                                                                                                                                                                                                                                                                    |
| ------------------------ | ------ | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `overmind.behaviour.key` | string | `@overmind.task(key)` (primary decorator or explicit dynamic context boundary) | The declared task this unit span executes. Stamped on the `entry_point` unit span; the server binds the trace to this Behaviour ahead of structural matching. A known key is strong evidence even when the git sha is missing, but stale revisions and incomplete anchors remain visible verification failures. Unknown keys are flagged (`declared_key_unknown`), never silently guessed. |
...
| `overmind.task.mode`     | string | `@overmind.task(..., mode="primary"|"child")`             | Declares whether the entry span owns the primary trace boundary or is an explicitly independent nested task. Primary tasks cannot be nested inside another primary task. |
