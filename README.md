...
`/overmind-optimize-agent` runs the optimization loop. It can launch `overmind optimize` in a fresh IDE terminal, or use host-driven `optimize-step` primitives for a more agent-native workflow.

## CLI quickstart

Use this path if you want to run Overmind directly from the terminal. If you use Cursor, Codex, or Claude Code, the Agent Skills workflow is recommended.
...
This creates `.overmind/` in your project root and prompts for API keys and default models. It is safe to re-run.
...
Point Overmind at the Python function it should call for each test case. The function receives an input dict and returns a dict:
...
For framework-based agents, create a small wrapper function that exposes this `dict -> dict` contract.
...
This analyzes your code, generates or imports a policy, builds the evaluation spec, and prepares the dataset.

You can also provide an existing policy document:

```bash
overmind setup my-agent --data data/seed.json --policy docs/my_policy.md
```

Use defaults without prompts:

```bash
overmind setup my-agent --data data/seed.json --fast
```
...
Overmind runs your agent against the dataset, scores outputs, diagnoses failures, generates candidate improvements, and keeps only changes that improve the score.

### Optional: validate the entrypoint

Before setup, you can run one test case through the registered entrypoint:

```bash
overmind agent validate my-agent --data data/seed.json
```
...
Configure API keys and default models. Writes `.overmind/.env` in the current directory. Safe to re-run.
...
| `overmind agent update <name> <mod:fn>` | Update the entrypoint |
...
| `--data PATH` | JSON seed dataset file or directory of `*.json` files. |
...
Other paths under `.overmind/` do not all exist until you run commands.
...
After `overmind setup`, `eval_spec.json` includes a `scope` block with two path lists, both relative to the project root:
...
overmind setup my-agent --scope "agents/core/*.py"
```
...
overmind optimize-step <subcmd>                      Step-wise primitives for host-driven optimization
